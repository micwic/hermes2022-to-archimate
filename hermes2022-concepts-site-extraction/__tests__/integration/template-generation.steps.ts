...
defineFeature(feature, (test) => {
  ...
  test('Génération de template depuis instructions', ({ given, when, then, and }) => {
    let config;
    let apiKey;
    let template;
    ...
    when('on génère un template NuExtract', async () => {
  ...
  test('Génération de template avec fallback sur schéma vide', ({ given, when, then, and }) => {
    given('des instructions de transformation markdown', () => {
      const configPath = resolveFromRepoRoot('hermes2022-concepts-site-extraction/config/extraction-config.json');
      expect(fs.existsSync(configPath)).toBe(true);
    });

    given('un schéma JSON inaccessible', () => {
      // Simuler un schéma inaccessible en modifiant temporairement la configuration
      // ou en mockant les fonctions de lecture de fichier
    });

    when('on génère un template NuExtract avec gestion d\'erreur', async () => {
      await loadGlobalConfig();
      await loadApiKey();
      // La génération devrait gérer l'erreur et utiliser un fallback
    });

    then('un avertissement est généré', () => {
      // Vérifier qu'un avertissement a été loggé
      expect(true).toBe(true); // Placeholder - à implémenter
    });

    and('le template utilise un schéma vide comme fallback', () => {
      // Vérifier que le template utilise un schéma vide
      expect(true).toBe(true); // Placeholder - à implémenter
    });

    and('le processus continue sans interruption', () => {
      // Vérifier que le processus n'a pas été arrêté
      expect(true).toBe(true); // Placeholder - à implémenter
    });
  });
});
